...
class Megas(Dataset):
    ...
    def __init__(self, path,window_size,trans=torch.Tensor,collectDataPercent=1,mode="train",Normalize=False,labelProbabaility=False,seed=None):
        ...
        self.augmentor=Augmenetor(Normalize=Normalize)
    ...
    def loadDataFormat(self, DataPath, window_size, transform, collectDataPercent):
        ...
        negativeAug = {i: [negativeAug[j] for j in range(len(negativeAug)) if i != j] for i in range(len(np.unique(Sl_Labels)))}
...
if __name__ == '__main__':
    st=time.time()
    mgs=Megas("../data/",44,Normalize=True)
