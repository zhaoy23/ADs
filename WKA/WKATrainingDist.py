...
from DataPattrens import getPattren
...
DataIndex=3
Epochs=500
distibutionSmall, distibutionLarge, distlist, lpercent, normalize, requiredSize, seed,_=getPattren(DataIndex)
...
DatasetName=f"WKA_{'_'.join(distibutionSmall)}___{'_'.join(distibutionLarge)}___{'__'.join(distlist)}___{lpercent}_{normalize}_{seed}_{requiredSize[0]}"
...
def AugmentData(S,windows_size=44):
    df, sequence, _ = data_quantile(S)
    features = [col for col in S.columns if col not in ("Sequence",)]
    data = S.loc[:, features]
    aug_data = data_augmentaion(data, sequence, windows_size,features)
...
def generateDataset():
    # SSize, Lsize = requiredSize[0],int(requiredSize * lpercent // 100)
    SSize, Lsize = int(requiredSize[0] * (100 - lpercent) / 100), int(requiredSize[0] * lpercent / 100)
    Sdata, _, _ = generateData("small", SSize, distibutionSmall, distibutionLarge, dist2normallist,
                               dist1abnormallist, dist2normallist,dist2abnormallist, seed=seed)
    Ldata, _, _ = generateData("large", Lsize, distibutionSmall, distibutionLarge,
                               dist2normallist,dist1abnormallist, dist2normallist, dist2abnormallist, seed=seed)
    Sdata=AugmentData(pd.DataFrame(Sdata,columns=['X', 'Y', 'Z']))
    Ldata=AugmentData(pd.DataFrame(Ldata,columns=['X', 'Y', 'Z']))
    data=np.concatenate([Sdata,Ldata],axis=0)
    return data

trainDataAug=generateDataset()
...
if os.path.exists(modelpath+modelname+".weights.h5"):
    ...
    try:
        ...
        tf.keras.models.save_model(model, modelpath + modelname + ".h5")
    model.save_weights(modelpath+modelname+".weights.h5")
    print("Model Saved at", modelpath+modelname)
